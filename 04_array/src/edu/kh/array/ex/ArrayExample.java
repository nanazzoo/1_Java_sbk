...
//		int형 변수 4개를 묶음으로 다룰 수 있는 배열을 생성하고 이를 'arr'이라고 부름
...
//		int[]을 참조하는 변수 arr 선언
		...
		arr = new int[4];
//		Heap 영역에 int 4칸 짜리 배열을 할당(생성)하고
//		할당 된 배열의 시작 주소를 arr에 대입
...
//		The local variable a may not have been initialized(초기화x)
...
//		3. 할당된 배열의 시작 주소를 arr 변수에 대입
	...
	//		점심 메뉴 뽑기 프로그램
		...
		String[] menuArr = {"김밥+라면", "KFC", "맘스터치", "써브웨이", 
				"백반", "순대국", "곰탕", "파스타", "쉐이크", "삼겹살"};
...
//		최고 / 최저점 구하기
		...
		int max = scoreArr[0];
		int min = scoreArr[0];
//		배열의 첫 번째 인덱스 값을 최고/최저의 기준으로 삼음
...
//		for문에 breakpoint를 지정하면 증감식이 해석되기 전에 멈춘다(debug mode)
		...
		for(int i=1; i<scoreArr.length; i++) {
			...
			if(scoreArr[i] > max) { //기준 최고점 보다 scoreArr[i] 값이 더 큰 경우
				max = scoreArr[i];
			} 
...
//		입력 받은 정수가 배열에 존재하면 몇 번 인덱스에 존재하는 지 출력
//		단, 없다면 "존재하지 않습니다" 출력
...
//		arr 배열에 대한 순차 접근(반복 접근)
		for(int i=0; i<arr.length; i++) {
			
			if(arr[i] == search) { //arr[i]번 째 값이 입력 받은 값과 같다면
...
//		Arrays.sort(배열명): 배열 내 값을 오름차순으로 정렬
	...
	public void ex9( ) {
//		얕은 복사
//			-참조하는 주소만을 복사하여 서로 다른 참조 변수가 
//			 하나의 배열(또는 객체)를 참조하게 하는 복사 방법
...
//		복사본의 값을 변경 
		copyArr[2] = 10000;
//		-> 원본은 값이 변하지 않음 == 데이터 공유X -> 서로 다른 배열
