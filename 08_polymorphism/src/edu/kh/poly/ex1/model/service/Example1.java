...
//	다형성: 상속을 이용한 기술로 부모 타입의 참조 변수 하나가
//			여러 타입의 자식 객체를 참조할 수 있음
	
	public void ex1() {
//		업 캐스팅(Up Casting): 부모 참조 변수 = 자식 객체
...
//		System.out.println(c1.toString());
		...
		동적 바인딩: 프로그램 실행 중 참조하는 객체 실제 타입의 오버라이딩된 메서드 연결
 		*
		*/
...
//		Car 참조 변수
		...
		arr[1] = new Truck(8, 3, "경유", 5.0); //Truck 객체(업 캐스팅)
//		Car 참조 변수
...
//		전달 받은 객체가 자식 객체이고 toString()을 Overriding 했다면
...
//		  부모 타입 참조 변수 -> 자식 타입 참조 변수로 변환(강제 형변환, 얕은 복사)
...
//			-> 다운 캐스팅이 잘못된 경우 발생
//		* 해결 방법: instanceof 연산자로 다운 캐스팅 할 타입이 맞는 지 검사
